...
const ManageBooks = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    fetch(`https://lit-tor-54339.herokuapp.com/tickets`)
      .then((res) => res.json())
  ...
  const updateStatus = (id) => {
    ...
    fetch(`https://lit-tor-54339.herokuapp.com/update/${id}`, {
      method: "PUT",
      headers: {
  ...
  const deleteTicket = (id) => {
    ...
    if (confirmation) {
      const url = `https://lit-tor-54339.herokuapp.com/myOrders/${id}`;
      fetch(url, {
        method: "DELETE",
      })
        .then((res) => res.json())
